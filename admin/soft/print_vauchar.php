...
<?php 
    ...
    $cur = date( "Y-m-d" ) ;
    $this_month123 = explode("-", $cur ) ;
    $this_month = $this_month123[1] ;
    $present = "" ;
    $all_data = "" ;
    $master_value = $_GET['master_no'] ;
    if( isset( $_GET['master_no'] ) ) 
    {
        $master_no = $_GET['master_no'] ;
        $sql = "SELECT member_profiles.FULL_NAME, member_profiles.MEMBER_ID, member_profiles.PRESENT_HOUSE_NO, member_profiles.PRESENT_LANE_NO, member_profiles.PRESENT_ROAD_NO, member_profiles.PRESENT_THANA, member_profiles.PRESENT_DISTRICT FROM member_depsoit_masters LEFT JOIN member_profiles ON member_profiles.PROFILE_NO = member_depsoit_masters.PROFILE_NO WHERE member_depsoit_masters.DEPOSIT_MASTER_NO = '$master_no' " ;

        $all_data = mysqli_fetch_assoc ( mysqli_query( $con , $sql ) ) ;
        $present = $all_data['PRESENT_HOUSE_NO'].", ".$all_data['PRESENT_LANE_NO'].", ".$all_data['PRESENT_ROAD_NO'].", ".$all_data['PRESENT_THANA'].", ".$all_data['PRESENT_DISTRICT'] ;
   ...
   <form id="form-validation" style="padding:20px;" action="actions/test.php" method="post" class="form-horizontal form-bordered" novalidate="novalidate" name="form-validation" enctype="multipart/form-data">
                    ...
                    <div class = "col-sm-6">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                    <input type="text" disabled id="val_fullname" name="val_fullname"   value="<?php echo getMonth( $this_month ) ;?>" style="color:black">
...
<table class="table table-bordered table-hover table-responsive table-condensed table-striped dataTable col-xs-12 col-sm-12 col-md-6 col-lg-6 ">
	...
	<tbody id="recordList">
		...
		<?php 
		    ...
		    $sql = "SELECT * FROM member_deposit_details LEFT JOIN member_depsoit_masters ON member_depsoit_masters.DEPOSIT_MASTER_NO = member_deposit_details.DEPOSIT_MASTER_NO LEFT JOIN deposite_heads ON deposite_heads.DEPOSITE_HEAD_NO = member_deposit_details.DEPOSITE_HEAD_NO WHERE member_depsoit_masters.DEPOSIT_MASTER_NO = '$master_value' " ;
		    $result = mysqli_query( $con , $sql ) ;
        $count = 1 ;
        $total_amount = 0 ;
		    foreach( $result as $value )
		    {
            $total_amount += $value['DEPOSIT_AMOUNT'] ;

		        echo "<tr>" ;
		        echo "<td style = 'text-align: center;'>". $count ++ ."</td>" ;
		        echo "<td style = 'text-align: center;'>".$value['DEPOSITE_HEAD_NAME']."</td>" ;
		        echo "<td style = 'text-align: center;'>".$value['DEPOSIT_AMOUNT']."</td>" ;
	...
	<tfoot>
	    <tr >
	        ...
	        <td class = "weight" ><?php echo $total_amount ; ?></td>
	        <td class = "weight" ></td>
	    </tr>
	</tfoot>
